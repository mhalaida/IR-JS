        ...
        collArr.forEach(function (fileName, fileIndex) {
            ...
            data.forEach(word => {
                if (isNaN(word)) {
                    ...
                    if (invIndex[word][fileIndex + 1] == undefined) {
                        invIndex[word][fileIndex + 1] = [wordCount];
                    } else {
                        invIndex[word][fileIndex + 1].push(wordCount);
                    }
                }
            });
        });
        ...
        collArr.forEach(function (fileName, fileIndex) {
            ...
            for (let i = 0; i < data.length; i++) {
                if (isNaN(data[i])) {
                    ...
                    if (!invIndex[currToken].includes(fileIndex)) {
                        invIndex[currToken].push(fileIndex);
                    }
                }
            }
        });
